...
function quotesgenerate() {
    document.getElementById("quotes").innerHTML = `"${quoteData[index].quote}"`
    document.getElementById("author").innerHTML = `"${quoteData[index].author}"`
}

function prevIndex() {
    if(index == 0) {
        index = quoteData.length - 1;
    }
    else {
        index--;
    }
    quotesgenerate(index);
}
